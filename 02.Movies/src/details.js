...
async function  onDelete(e, id){
  e.preventDefault();
  const confirmed = confirm('Are you sure that you want to delete this movie?')
  if (confirmed){
    const response = await fetch('http://localhost:3030/data/movies/' + id, {
      method:'delete',
...
function createMovieCard(movie, likes, ownLike){
    ...
    if(userId != null){
      if(userId == movie._ownerId){
        ...
        controls.querySelector('.btn-danger').addEventListener('click', (e) => onDelete(e, movie._id));
        controls.appendChild(e('a', {className:'btn btn-warning', href:'#'}, 'Edit'));
        controls.querySelector('.btn-warning').addEventListener('click', (e) => showEdit(movie._id));
      }else if (ownLike.length == 0){
        controls.appendChild(e('a', {className:'btn btn-primary', href:'#'}, 'Like'));
        controls.querySelector('.btn-primary').addEventListener('click', likeMovie);
      }
    }
